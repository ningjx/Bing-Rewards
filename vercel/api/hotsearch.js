/**
 * Vercel兼容的热搜词聚合API
 * 支持获取各大平台热搜词，包括百度、抖音、今日头条、知乎、哔哩哔哩等
 */
...
const createApiInfos = () => [
  ...
  { name: "en", url: `https://api.finlight.me/v2/articles`, headers: { 'Content-Type': 'application/json', 'accept': 'application/json', 'X-API-KEY': process.env.Finlight_KEY }, body: "{\"pageSize\": \"100\"}", keyword: "title" }
];

// 从GitHub获取words.txt内容
async function getWordsFromTxt() {
  try {
    const response = await fetch('https://raw.githubusercontent.com/ningjx/Bing-Rewards/refs/heads/main/worker/static/words.txt');
    if (!response.ok) {
